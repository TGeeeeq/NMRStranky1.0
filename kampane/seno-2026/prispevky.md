# Kampaň „Seno pro Louku" 2026 — příspěvky na sociální sítě

**Cíl:** vybrat 100 000 Kč na zimní zásobu sena (~125 balíků při 800 Kč/balík).
**Odkaz kampaně:** https://nechmerust.org/seno
**Platba:** QR na stránce, transparentní účet 2002645872/2010, VS 20260716, nebo Darujme.cz.

**Potvrzená čísla (od spolku):**
- menší balík (manipulace v jednom člověku): loni **500 Kč**, letos odhad **800–1 000 Kč i víc**; velké balíky byly loni ~700 Kč
- spotřeba: **2 mladé krávy à ~15 kg/den**, **oslík Karel ~10 kg/den**, 8 ovcí + 2 letošní jehňata, muflon Yakul, prasátka, králíci
- bonus: dar **200 Kč a víc** = po napsání na info@nechmerust.org přístup ke hře **Louka Run**

---

## 1. Launch post (den 1 — Instagram + Facebook)

### Varianta A — vyprávěcí (doporučená pro FB)

> Seno je letos vzácné. 🌾
>
> Letošní rok přinesl nejhorší sucho v historii měření. Seno pro naše stádo každý rok kupujeme — jenže letos ho je málo v celém kraji a cena vyletěla: balík, který jsme loni brali za 500 Kč, letos stojí 800 až 1 000 korun, možná i víc. Proto nečekáme a zásobu sháníme už teď.
>
> A stádo má apetit: krávy Avala a Květa spořádají každá kolem 15 kilo sena denně, oslík Karel dalších deset. K tomu osm ovcí, dvě letošní jehňátka, muflon Yakul, prasátka i králíci — všichni musí žrát celou zimu, ať je seno za kolik chce.
>
> Proto spouštíme sbírku **Seno pro Louku**. Vybíráme 100 000 Kč, tedy zhruba 125 balíků. Každá koruna projde naším transparentním účtem přímo na seno.
>
> 🌾 800 Kč = jeden balík sena
> 💚 Přispět můžete QR platbou nebo kartou: nechmerust.org/seno
> 🔁 A pokud přispět nemůžete, sdílejte — i to nám může přivézt balík.
>
> Loni jste nám se senem pomohli. Věříme, že to spolu zvládneme i letos. Děkujeme! 💚

Hashtagy: #nechmerust #senoprolouku #sucho #sbirka #pomoczviratum #louka #azyl #zvirata #transparentne

### Varianta B — úderná (doporučená pro IG)

> 🌾 SPOUŠTÍME SBÍRKU: Seno pro Louku
>
> Seno pro naše stádo každý rok kupujeme. Po letošním suchu ho je ale málo v celém kraji a balík zdražil z loňských 500 Kč na 800 až 1 000 Kč. Připravujeme se na zimu s předstihem.
>
> Naše stádo potřebuje na zimu ~125 balíků. Vybíráme 100 000 Kč.
>
> 🌾 400 Kč = půl balíku
> 🌾 800 Kč = celý balík
> 🌾 1 600 Kč = dva balíky
>
> Odkaz v biu → nechmerust.org/seno
> Každá koruna je vidět na transparentním účtu. Děkujeme! 💚

Hashtagy: #nechmerust #senoprolouku #sucho #sbirka #pomoczviratum #zvireciazyl #darujme

### Varianta C — otázková (engagement)

> Víte, kolik sena spořádá jeden oslík za zimu? 🫏
>
> Karel zvládne zhruba 10 kg denně — a naše dvě mladé krávy každá ještě o pět kilo víc. Seno pro ně každý rok kupujeme, jenže po letošním suchu je vzácné: platí se 800 až 1 000 Kč za balík, který loni stál pětistovku.
>
> Proto vybíráme 100 000 Kč na zimní zásobu: nechmerust.org/seno
>
> Kolik balíků pošleme Karlovi společně? Napište do komentářů, kdo jste přispěli — ať víme, komu poděkovat! 👇💚

---

## 2. Story série (den 1–2, Instagram Stories)

1. **Story 1 — situace:** foto vyprahlé louky (např. z hero fotky Karla). Text: „Letošní sucho udělalo ze sena vzácné a drahé zboží. 🌾" + nálepka s otázkou „Věděli jste, že letos je nejhorší sucho v historii měření?"
2. **Story 2 — dopad:** foto zvířat. Text: „Karel, Avala, Květa, Pogo, Lucinka… všichni potřebují seno na celou zimu. ~125 balíků. Balík letos stojí min. 800 Kč."
3. **Story 3 — řešení:** obrázek QR kódu (`web/public/assets/seno-qr.png`). Text: „Namiřte fotoaparát bankovní aplikace 👆 nebo odkaz: nechmerust.org/seno" + link nálepka na /seno.
4. **Story 4 — díky/sdílení:** „Každá koruna je vidět na transparentním účtu. Sdílejte tuhle story — i to pomáhá. 💚"

Tip: Stories 1–4 uložit do výběru (highlight) „Seno 2026".

---

## 3. Příběhový post (den 3–4)

> Tohle je Karel. 🫏
>
> Hravý oslík s velkým srdcem, který miluje drbání ve stínu. Karel toho po nás moc nechce — stín, vodu a v zimě pořádnou porci sena. Denně spořádá zhruba 10 kilo; přes celou zimu je to víc než tuna sena. A to je jen jeden obyvatel Louky.
>
> Seno pro Karla a spol. každý rok kupujeme. Jenže letos je po suchu vzácné a balík, který loni stál 500 Kč, vyjde na 800 až 1 000 korun.
>
> 800 Kč = jeden balík sena navíc ve stodole.
>
> Pomůžete Karlovi (a celému stádu) naplnit stodolu? → nechmerust.org/seno 💚
>
> P.S. Za dar 200 Kč a víc vám rádi dáme přístup k naší hře Louka Run — stačí nám napsat. Karel v ní běhá taky. 🎮

Hashtagy: #nechmerust #senoprolouku #osel #karel #sbirka #pomoczviratum

---

## 4. Milníkové updaty (šablony)

### 25 % / 50 % / 75 %

> 🌾 {X} % je doma! Díky vám máme vybráno {ČÁSTKA} Kč z cílových 100 000 Kč — to je už {POČET} balíků sena ve stodole.
>
> Děkujeme každému, kdo přispěl nebo sdílel. Jedeme dál → nechmerust.org/seno 💚

### 100 % — poděkování

> DOKÁZALI JSTE TO! 💚🌾
>
> 100 000 Kč na seno je vybráno. Naše stodola se plní a Karel, Avala, Květa i všichni ostatní mají zimu zajištěnou. Tohle je vaše zásluha — každého dárce i každého, kdo sdílel.
>
> Všechny pohyby vidíte na transparentním účtu: https://ib.fio.cz/ib/transparent?a=2002645872
>
> DĚKUJEME. Jste naše smečka. 🐾

**Postup při updatu:** upravit `raised` a `updatedAt` v `web/lib/campaign.ts`, commitnout a pushnout na `main` — web (stránka /seno i banner) se aktualizuje automaticky.

---

## 5. Harmonogram spuštění

| Den | Akce |
|---|---|
| Den 1 | Nasadit web (push na main) → launch post (A na FB, B na IG) + Stories 1–4 |
| Den 2 | Karusel ze studia (šablona `seno-pro-louku` ve /studio) |
| Den 3–4 | Příběhový post (Karel) |
| Průběžně | Aktualizace částky v `web/lib/campaign.ts` + milníkové posty (25/50/75/100 %) |
| Po naplnění | Poděkování + foto plné stodoly / navážky sena |

**Tipy navíc:**
- Přidat odkaz nechmerust.org/seno do bia na Instagramu i Facebooku.
- Poprosit fanoušky Louka Run a odběratele newsletteru (pokud existuje) o sdílení.
- Při každé navážce sena natočit krátké video/reels — reálné záběry táhnou víc než grafika.
- Komunikovat bonus: **dar 200 Kč a víc = přístup ke hře Louka Run** (dárce napíše na info@nechmerust.org). Hra je i na Google Play — koupě hry je další forma podpory.
